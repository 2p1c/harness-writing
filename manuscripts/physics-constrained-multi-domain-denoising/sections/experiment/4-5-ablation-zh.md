...
\textbf{仅时域分支}用单一时域编码器-解码器（容量与一个PMDF-Net分支相当，即参数量大致相等）替代双分支结构。训练仅使用MSE损失，无时频分支或物理约束。此配置量化了多域融合相对于单域基线的优势。

\textbf{仅时频分支}镜像仅时域分支变体但仅在STFT幅度频谱上运行。2D卷积编码器-解码器独立处理频谱，逆STFT重建去噪后的时域波形。应用MSE损失但无物理约束。此变体测量独立时频去噪相对于时域处理的性能。
