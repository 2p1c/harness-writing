...
与传统信号平均（无差别保留所有特征）不同，神经网络可能学习到不随试样条件转移的数据集特定伪影。
我们通过完全在完整铝板获取的信号上训练PMDF-Net，随后在含缺陷试样信号上测试（无任何微调）来评估跨域泛化能力。
...
\textbf{泛化为何有效。}物理约束损失显式惩罚物理意义信号特征的偏离——到达时间偏移、幅度变化和波形失真——而非仅优化像素级或语谱图级重建指标。
因为缺陷表现为这些相同物理特征的扰动（由散射引起的到达时间改变、由能量耗散引起的幅度减小、由模态转换引起的波形失真），
训练以保留物理有意义结构的模型隐式地被训练为对缺陷相关扰动敏感。
多域融合通过同时捕获时域波形细节和时频谱特征来补充这一点，确保在一个表示中可见但在另一表示中被遮蔽的缺陷特征仍能被保留。
