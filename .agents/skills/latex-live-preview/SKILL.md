...
which node      # Node.js runtime (for pdf-live-server)
```
...
The `pdf-live-server` is bundled in `scripts/pdf-live-server.js` (Node.js) and uses file polling — no external file watchers required.
